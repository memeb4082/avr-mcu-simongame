...
int main()
{
    ...
    // Stores SPI thingo
    uint8_t spi;
    // Stores index of playback output
    uint32_t idx = 0;
    // Stores index of user input playback
    uint32_t u_idx = 0;
    // Stores input state of user corresponding step from rng algorithm
    uint8_t input;
    // Bool var sorta
    while (1)
    {
        ...
        */
        ...
        case FAIL:
        {
            ...
            if ((elapsed_time >= WAIT_FAIL) | (SERIAL_STATE == AWAITING_COMMAND))
            {
                ...
                if (score >= lowest)
                {
                    // iterates and appends/overwrites to table where
                    // empty or new score is greater than previous score
                    for (int i = 0; i < SCORE_TABLE_SIZE; i++)
                    {
                        if ((names[i][0] == '\0') | ((level > scores[i])))
                        {
                            int j = 0;
                            while (name[j] != '\0')
                            {
